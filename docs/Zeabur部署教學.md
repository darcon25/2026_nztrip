...
**⚠️ 實測發現：Zeabur 常常會把這個專案誤判成「純靜態網站」**，因為專案裡有 Vite 設定，它偵測到 Vite 就直接當成靜態網站處理，完全不執行 `npm run start`、也不會跑 `server.ts` 這個後端。**症狀是**：網站首頁看起來正常，但所有 `/api/*` 開頭的功能（照片、留言、分帳、自煮分工填菜色）其實都是假的——打開網路開發者工具會看到這些網址回傳的內容是網站首頁的 HTML，不是真的資料；嘗試寫入資料會被拒絕（405 錯誤）。

**修法**：到「**環境變數**」分頁，多加一筆：

```
ZBPACK_PLAN_TYPE=nodejs
```

這會強制 Zeabur 用「跑一個 Node.js 服務」的方式部署，而不是「純靜態網站」。加完存檔、觸發重新部署（通常會自動觸發，沒有的話手動點 Redeploy）。

**如果部署完打開網址是空白頁或錯誤畫面**，才需要手動指定啟動指令：
...
| **首頁看起來正常，照片、留言、分帳、填菜色全部都不能用**（這是最容易誤判的一種，表面上網站是好的） | Zeabur 把整個專案當成純靜態網站部署，`server.ts` 後端根本沒在跑。回步驟 2 加 `ZBPACK_PLAN_TYPE=nodejs` 這個環境變數，重新部署 |
